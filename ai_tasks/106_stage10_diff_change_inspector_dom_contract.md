# AI Task 106 — Stage 10 Diff Change Inspector DOM Contract

## Stage
Stage 10 — Execution Entry

## Substage
Diff interaction refinement above the Stage 10 inspector preview integration baseline

## Goal
Добавить stable DOM contract для inspector-derived diff detail rows, чтобы comparison-ready diff section был пригоден для следующего UI/interaction шага и проверялся machine-readable HTML markers, а не только визуально.

## Why This Matters
После AI Task 105 changed-key inspector уже встроен в preview UI, но DOM still lacks a stable row-level contract for future interactions and scan-safe validation. Этот task должен закрепить machine-readable DOM markers без нового orchestration wrapper.
...
## Requirements
- The diff preview must continue consuming `get_stage10_diff_change_inspector_contract.sh` as the changed-key interaction artifact.
- Comparison-ready inspector rows must expose stable DOM markers for per-key scan-safe verification.
- It must not rebuild lower transition layers.
- It must not run benchmark in the ordinary path.
- It must preserve the existing comparison-ready baseline from AI Tasks 102–105.
...
## Acceptance Criteria
- `verify_stage10_diff_change_inspector_dom_contract.sh` returns exactly one JSON object.
- The verifier passes on the live comparable project.
- The preview diff section exposes stable DOM markers for inspector-derived per-key detail rows.
- README and recovery state document this DOM-contract step as the next interaction/UI artifact above the inspector preview integration baseline.

## Acceptance Model
- Primary acceptance gate: `verify_stage10_diff_change_inspector_dom_contract.sh`
...
bash -n code/ui/verify_stage10_diff_change_inspector_dom_contract.sh

bash code/ui/verify_stage10_diff_change_inspector_dom_contract.sh --help 2>&1 | grep -q "DOM contract"
```
...
bash code/ui/verify_stage10_diff_change_inspector_dom_contract.sh --project-id "$PROJECT_ID" --output-dir /tmp/contextviewer_ui_preview
```
Expected:
- verifier JSON has `status: "pass"` and `failed_checks: 0`
- checks confirm inspector-derived per-key DOM markers and key/type parity with the Stage 10 inspector contract

### Visual (Layer 4)
Executor: user. Confirm no visual regression while DOM contract markers are added.
```bash
export PROJECT_ID=18
export STAGE9_GATE_TIMEOUT_S=60
...
- confirmed checklist:
  ...
  - [ ] changed-key inspector area remains visible
  - [ ] per-key detail rows remain visible
...
```
Expected: contract JSON only; not part of ordinary DOM-contract acceptance.
