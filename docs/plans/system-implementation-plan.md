...
## Current Substage
Stage **127** delivered the **shell and navigation** **release-candidate** productization (**`127`** on **`<body>`**, **`cv-app-shell--product-rc`**, clearer nav chrome) while preserving **`080`**, all section markers, and payload embedding; lightweight validation model continues with the next larger product-facing slice
