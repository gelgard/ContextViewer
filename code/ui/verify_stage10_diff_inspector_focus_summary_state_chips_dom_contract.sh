#!/usr/bin/env bash
# AI Task 112: Stage 10 diff inspector focus-summary state-chips DOM-contract verifier.
set -euo pipefail
...
usage() {
  cat <<'USAGE'
verify_stage10_diff_inspector_focus_summary_state_chips_dom_contract.sh — Stage 112 state-chips DOM contract

Validates stable DOM markers on the state-chip strip (112 contract) + per-chip field/value span markers vs default-focused row. No benchmark.
...
if [[ -f "$html" ]]; then
  ...
  if grep -q 'data-cv-inspector-rows-dom-contract="106"' "$html" 2>/dev/null; then
    if ! grep -q 'data-cv-diff-inspector-focus-summary-state-chips="111"' "$html" 2>/dev/null \
      ...
      || ! grep -q 'data-cv-diff-inspector-focus-summary-source-link-dom-fields="114"' "$html" 2>/dev/null; then
...
if [[ ! -s "$html_tmp" ]]; then
  add_check "html: workspace state-chips DOM contract (112)" "fail" "missing HTML preview artifact"
  add_check "html: strip + per-chip state-chip-field / state-chip-value vs default row" "fail" "missing HTML preview artifact"
else
  if [[ "$effective_row_count" -eq 0 ]]; then
    add_check "html: workspace state-chips DOM contract (112)" "pass" "skipped (zero changed-key inspector rows)"
    add_check "html: strip + per-chip state-chip-field / state-chip-value vs default row" "pass" "skipped (zero changed-key rows)"
  else
    if grep -q 'data-cv-diff-inspector-focus-summary-state-chips-dom-contract="112"' "$html_tmp" 2>/dev/null; then
      add_check "html: workspace state-chips DOM contract (112)" "pass" \
        'data-cv-diff-inspector-focus-summary-state-chips-dom-contract="112"'
    else
      add_check "html: workspace state-chips DOM contract (112)" "fail" "missing Task 112 state-chips DOM contract on workspace"
    fi
    py_c="$(
...
if not re.search(
    r'<div class="diff-inspector-focus-summary-chips"[^>]*data-cv-diff-inspector-focus-summary-state-chips-dom-contract="112"',
    page,
):
    print("fail|missing 112 DOM contract on state-chip strip container")
    sys.exit(0)
...
for name, val in (
    ...
    outer = (
        ...
        r'\s+data-cv-inspector-focus-summary-state-chip-field="' + re.escape(name) + r'"'
    )
    if not re.search(outer, page):
        print(f"fail|missing or mismatch state-chip-field block for {name}")
        sys.exit(0)
    inner = (
        r'<span class="diff-inspector-state-chip-val mono"\s+'
        r'data-cv-inspector-focus-summary-state-chip-value="' + re.escape(ev) + r'"'
    )
    if not re.search(inner, page):
        print(f"fail|missing state-chip-value span for {name}")
        sys.exit(0)
...
PY
    ...
    if [[ "$py_c" == "ok" ]]; then
      add_check "html: strip + per-chip state-chip-field / state-chip-value vs default row" "pass" \
        "112 strip + four chips match first changed_key_inspector row"
    else
      add_check "html: strip + per-chip state-chip-field / state-chip-value vs default row" "fail" "${py_c#fail|}"
    fi
  fi
fi
