#!/usr/bin/env bash
# AI Task 130: Stage 10 history surface productization (release-candidate preview gate).
set -euo pipefail
...
usage() {
  cat <<'USAGE'
verify_stage10_history_surface_productization_release_candidate.sh — Stage 130 history surface RC (live HTML)

Regenerates preview via prepare_ui_preview_launch.sh and validates Task 130 history productization markers
on the feed-backed history HTML (no benchmark).

Prints exactly one JSON object:
  status, checks, failed_checks, generated_at

Requirement mapping (declarative): PG-HI-001, PG-HI-002, PG-UX-001, PG-EX-001, PG-RT-001, PG-RT-002.
...
add_check "traceability: product goal IDs (declarative)" "pass" \
  "PG-HI-001 PG-HI-002 PG-UX-001 PG-EX-001 PG-RT-001 PG-RT-002"
...
if [[ ! -s "$html_tmp" ]]; then
  ...
  if grep -q 'data-section="history"' "$html_tmp" 2>/dev/null; then
    add_check "html: data-section history retained" "pass" 'data-section="history"'
  else
    add_check "html: data-section history retained" "fail" "missing history section root"
  fi
  if grep -q 'data-cv-history-surface="083"' "$html_tmp" 2>/dev/null; then
    add_check "html: Task 083 history baseline retained" "pass" 'data-cv-history-surface="083"'
  else
    add_check "html: Task 083 history baseline retained" "fail" "missing 083 history marker"
  fi
  if grep -q 'data-cv-history-surface-productization="130"' "$html_tmp" 2>/dev/null; then
    add_check "html: Task 130 history productization marker" "pass" 'data-cv-history-surface-productization="130"'
  else
    add_check "html: Task 130 history productization marker" "fail" "missing Task 130 marker"
  fi
  if grep -q 'history-workspace--product-rc' "$html_tmp" 2>/dev/null; then
    add_check "html: product RC history class" "pass" "history-workspace--product-rc"
  else
    add_check "html: product RC history class" "fail" "missing history-workspace--product-rc"
  fi
  if grep -q 'class="history-workspace"' "$html_tmp" 2>/dev/null; then
    add_check "html: history-workspace root retained" "pass" "history-workspace"
  else
    add_check "html: history-workspace root retained" "fail" "missing history-workspace"
  fi
  if grep -q 'class="hist-product-hero"' "$html_tmp" 2>/dev/null; then
    add_check "html: hist product hero (130)" "pass" "hist-product-hero"
  else
    add_check "html: hist product hero (130)" "fail" "missing hist-product-hero"
  fi
  if grep -q 'id="cv-section-history"' "$html_tmp" 2>/dev/null; then
    add_check "html: history section id retained" "pass" "cv-section-history"
  else
    add_check "html: history section id retained" "fail" "missing cv-section-history"
  fi
