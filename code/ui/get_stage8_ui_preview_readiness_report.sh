...
if [[ "$mode" == "full" ]]; then
  ...
  if [[ -n "$output_file_fast" && -f "$output_file_fast" ]]; then
    ...
    if grep -q 'data-section="overview"' "$output_file_fast" 2>/dev/null; then
      ...
      if grep -q 'data-cv-overview-surface-productization="128"' "$output_file_fast" 2>/dev/null \
        && grep -q 'overview-surface--product-rc' "$output_file_fast" 2>/dev/null; then
        add_fast_check "delivery-fast: overview surface productization (128)" "pass" "Task 128 markers present"
      else
        add_fast_check "delivery-fast: overview surface productization (128)" "fail" "regenerate preview for Task 128 overview RC markers"
      fi
    else
      add_fast_check "delivery-fast: overview marker" "fail" 'missing data-section="overview"'
      add_fast_check "delivery-fast: overview surface productization (128)" "pass" "skipped (no overview section)"
    fi
    ...
    if grep -q 'data-section="history"' "$output_file_fast" 2>/dev/null; then
      ...
      if grep -q 'data-cv-history-surface-productization="130"' "$output_file_fast" 2>/dev/null \
        && grep -q 'history-workspace--product-rc' "$output_file_fast" 2>/dev/null \
        && grep -q 'data-cv-history-surface="083"' "$output_file_fast" 2>/dev/null; then
        add_fast_check "delivery-fast: history surface productization (130)" "pass" "Task 130 markers present"
      else
        add_fast_check "delivery-fast: history surface productization (130)" "fail" "regenerate preview for Task 130 history RC markers"
      fi
    else
      add_fast_check "delivery-fast: history marker" "fail" 'missing data-section="history"'
      add_fast_check "delivery-fast: history surface productization (130)" "pass" "skipped (no history section)"
    fi
    ...
    if [[ "$prep_cmp" == "true" ]]; then
      if grep -q 'data-cv-diff-inspector-focus-summary="108"' "$output_file_fast" 2>/dev/null; then
        if grep -q 'data-cv-diff-inspector-focus-summary-source-link-hint-dom-contract="118"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-field="linked_key"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-field="linked_index"' "$output_file_fast" 2>/dev/null \
          && grep -q 'diff-inspector-focus-summary-source-hint' "$output_file_fast" 2>/dev/null; then
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint DOM contract (118)" "pass" "118 source-link hint DOM contract present"
        else
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint DOM contract (118)" "fail" "regenerate preview for Task 118 source-link hint DOM contract"
        fi
      else
        add_fast_check "delivery-fast: diff inspector focus-summary source-link hint DOM contract (118)" "pass" "skipped (no focus-summary block)"
      fi
    else
    ...
    if [[ "$prep_cmp" == "true" ]]; then
      if grep -q 'data-cv-diff-inspector-focus-summary="108"' "$output_file_fast" 2>/dev/null; then
        if grep -q 'data-cv-diff-inspector-focus-summary-source-link-hint-badge-dom-contract="120"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-field="badge_label"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-field="badge_value"' "$output_file_fast" 2>/dev/null \
          && grep -q 'diff-inspector-focus-summary-source-hint-badge' "$output_file_fast" 2>/dev/null; then
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge DOM contract (120)" "pass" "120 hint badge DOM contract present"
        else
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge DOM contract (120)" "fail" "regenerate preview for Task 120 hint badge DOM contract"
        fi
      else
        add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge DOM contract (120)" "pass" "skipped (no focus-summary block)"
      fi
    else
      add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge DOM contract (120)" "pass" "skipped (comparison_ready false)"
    fi
    if [[ "$prep_cmp" == "true" ]]; then
      if grep -q 'data-cv-diff-inspector-focus-summary="108"' "$output_file_fast" 2>/dev/null; then
        if grep -q 'data-cv-diff-inspector-focus-summary-source-link-hint-badge-copy="121"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-diff-inspector-focus-summary-source-link-hint-badge-copy-dom-contract="122"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-copy-field="readable_text"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-copy-field="readable_value"' "$output_file_fast" 2>/dev/null \
          && grep -q 'diff-inspector-focus-summary-source-hint-badge-copy' "$output_file_fast" 2>/dev/null \
          && grep -q 'Inspector row 0 (default focus)' "$output_file_fast" 2>/dev/null; then
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy (121)" "pass" "121 hint badge readable copy present"
        else
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy (121)" "fail" "regenerate preview for Task 121 hint badge copy"
        fi
      else
        add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy (121)" "pass" "skipped (no focus-summary block)"
      fi
    else
      add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy (121)" "pass" "skipped (comparison_ready false)"
    fi
    if [[ "$prep_cmp" == "true" ]]; then
      if grep -q 'data-cv-diff-inspector-focus-summary="108"' "$output_file_fast" 2>/dev/null; then
        if grep -q 'data-cv-diff-inspector-focus-summary-source-link-hint-badge-copy-dom-contract="122"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-copy-field="readable_value"' "$output_file_fast" 2>/dev/null \
          && grep -q 'data-cv-inspector-focus-summary-source-link-hint-badge-copy-value="' "$output_file_fast" 2>/dev/null; then
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy DOM contract (122)" "pass" "122 badge-copy DOM contract present"
        else
          add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy DOM contract (122)" "fail" "regenerate preview for Task 122 badge-copy DOM contract"
        fi
      else
        add_fast_check "delivery-fast: diff inspector focus-summary source-link hint badge copy DOM contract (122)" "pass" "skipped (no focus-summary block)"
      fi
    else
