#!/usr/bin/env bash
# AI Task 129: Stage 10 visualization surface productization (release-candidate preview gate).
set -euo pipefail
...
usage() {
  cat <<'USAGE'
verify_stage10_visualization_surface_productization_release_candidate.sh — Stage 129 visualization surface RC (live HTML)

Regenerates preview via prepare_ui_preview_launch.sh and validates Task 129 visualization productization markers
on the feed-backed viz HTML (no benchmark).

Prints exactly one JSON object:
  status, checks, failed_checks, generated_at

Requirement mapping (declarative): PG-AR-001, PG-AR-002, PG-UX-001, PG-EX-001, PG-RT-001, PG-RT-002.
...
add_check "traceability: product goal IDs (declarative)" "pass" \
  "PG-AR-001 PG-AR-002 PG-UX-001 PG-EX-001 PG-RT-001 PG-RT-002"
...
if [[ ! -s "$html_tmp" ]]; then
  ...
  if grep -q 'data-section="visualization"' "$html_tmp" 2>/dev/null; then
    add_check "html: data-section visualization retained" "pass" 'data-section="visualization"'
  else
    add_check "html: data-section visualization retained" "fail" "missing visualization section root"
  fi
  if grep -q 'data-cv-visualization-surface-productization="129"' "$html_tmp" 2>/dev/null; then
    add_check "html: Task 129 visualization productization marker" "pass" 'data-cv-visualization-surface-productization="129"'
  else
    add_check "html: Task 129 visualization productization marker" "fail" "missing Task 129 marker"
  fi
  if grep -q 'viz-workspace--product-rc' "$html_tmp" 2>/dev/null; then
    add_check "html: product RC visualization class" "pass" "viz-workspace--product-rc"
  else
    add_check "html: product RC visualization class" "fail" "missing viz-workspace--product-rc"
  fi
  if grep -q 'class="viz-workspace"' "$html_tmp" 2>/dev/null; then
    add_check "html: viz-workspace root retained" "pass" "viz-workspace"
  else
    add_check "html: viz-workspace root retained" "fail" "missing viz-workspace"
  fi
  if grep -q 'class="viz-product-hero"' "$html_tmp" 2>/dev/null; then
    add_check "html: viz product hero (129)" "pass" "viz-product-hero"
  else
    add_check "html: viz product hero (129)" "fail" "missing viz-product-hero"
  fi
  if grep -q 'id="cv-section-visualization"' "$html_tmp" 2>/dev/null; then
    add_check "html: visualization section id retained" "pass" "cv-section-visualization"
  else
    add_check "html: visualization section id retained" "fail" "missing cv-section-visualization"
  fi
