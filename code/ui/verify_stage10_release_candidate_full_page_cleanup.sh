#!/usr/bin/env bash
# AI Task 131: Stage 10 release-candidate full-page cleanup (preview gate).
set -euo pipefail
...
usage() {
  cat <<'USAGE'
verify_stage10_release_candidate_full_page_cleanup.sh — Stage 131 full-page RC cleanup (live HTML)

Regenerates preview via prepare_ui_preview_launch.sh and validates Task 131 page-level markers plus
Task 125–130 surface RC markers and section roots (no benchmark).

Prints exactly one JSON object:
  status, checks, failed_checks, generated_at

Requirement mapping (declarative): PG-OV-001, PG-AR-001, PG-HI-001, PG-UX-001, PG-EX-001, PG-RT-001, PG-RT-002.
...
add_check "traceability: product goal IDs (declarative)" "pass" \
  "PG-OV-001 PG-AR-001 PG-HI-001 PG-UX-001 PG-EX-001 PG-RT-001 PG-RT-002"
...
if [[ ! -s "$html_tmp" ]]; then
  ...
  if grep -q 'data-cv-release-candidate-full-page-cleanup="131"' "$html_tmp" 2>/dev/null; then
    add_check "html: Task 131 full-page cleanup marker" "pass" 'data-cv-release-candidate-full-page-cleanup="131"'
  else
    add_check "html: Task 131 full-page cleanup marker" "fail" "missing Task 131 marker"
  fi
  if grep -q 'cv-app-shell--rc-page-complete' "$html_tmp" 2>/dev/null; then
    add_check "html: RC page-complete shell class" "pass" "cv-app-shell--rc-page-complete"
  else
    add_check "html: RC page-complete shell class" "fail" "missing cv-app-shell--rc-page-complete"
  fi
  if grep -q 'data-cv-shell-navigation-productization="127"' "$html_tmp" 2>/dev/null \
    && grep -q 'cv-app-shell--product-rc' "$html_tmp" 2>/dev/null; then
    add_check "html: Task 127 shell RC retained" "pass" "127 + product-rc shell"
  else
    add_check "html: Task 127 shell RC retained" "fail" "missing 127 shell markers"
  fi
  surf_ok="true"
  surf_miss=""
  for m in \
    'data-cv-overview-surface-productization="128"' \
    'data-cv-visualization-surface-productization="129"' \
    'data-cv-history-surface-productization="130"' \
    'data-cv-diff-surface-productization="125"' \
    'data-cv-settings-surface-productization="126"'; do
    if ! grep -qF "$m" "$html_tmp" 2>/dev/null; then
      surf_ok="false"
      surf_miss="$m"
      break
    fi
  ...
  if [[ "$surf_ok" == "true" ]]; then
    add_check "html: Task 125–130 surface markers" "pass" "128 129 130 125 126"
  else
    add_check "html: Task 125–130 surface markers" "fail" "missing ${surf_miss:-marker}"
  fi
  sec_ok="true"
  ...
  if [[ "$sec_ok" == "true" ]]; then
    ...
    add_check "html: all data-section roots" "fail" "missing data-section"
  fi
  id_ok="true"
  ...
  if [[ "$id_ok" == "true" ]]; then
    add_check "html: section ids retained" "pass" "all cv-section-* ids"
  else
  ...
  if grep -q 'id="ui-bootstrap-payload"' "$html_tmp" 2>/dev/null; then
    ...
    add_check "html: embedded payload retained" "fail" "missing payload"
  fi
fi
