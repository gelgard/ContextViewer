#!/usr/bin/env bash
# AI Task 019: current status projection from latest valid snapshot (read-only).
set -euo pipefail
...
usage() {
  cat <<'USAGE'
get_latest_current_status_projection.sh — current status from latest valid snapshot

Usage:
  get_latest_current_status_projection.sh <project_id>

Selects the newest snapshots row for the project where is_valid = true, ordered by
filename-derived "timestamp" DESC, "id" DESC. Projects raw_json.progress and
raw_json.changes_since_previous into a stable JSON shape.
...
Stdout:
  One JSON object:
    ...
    current_status         object:
      ...
      Missing or wrong types become []; if progress is not an object, the first
      three are []. changes_since_previous is [] unless the key is a JSON array.

If there is no valid snapshot: latest_snapshot_id null, all arrays empty, exit 0.
...
jq -n \
  ...
  def norm_changes_since_previous($L):
    if ($L | type) == "object" and ($L.changes_since_previous | type) == "array" then
  ...
  | if $lid == null then
      {
        ...
        current_status: empty_current_status
      }
    else
      (
        if ($L | type) == "object" then norm_progress($L.progress) else empty_progress end
      ) as $prog
    | (if ($L | type) == "object" then norm_changes_since_previous($L) else [] end) as $ch
    | {
        project_id: $pid,
